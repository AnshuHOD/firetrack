...
export async function sendIncidentReport(periodHours: number = 3) {
  ...
  if (!newLeads || newLeads.length === 0) {
    ...
    return { success: true, count: 0, message: "No new leads" };
  }
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; color: #333;">
      <h2 style="color: #d32f2f; border-bottom: 2px solid #d32f2f; padding-bottom: 8px;">🔥 FireLeadTracker — ${periodHours}-Hour Report</h2>
      ...
      <div style="background: #fff3e0; padding: 16px; border-radius: 8px; margin: 16px 0; border-left: 4px solid #f97316;">
        <strong>Summary:</strong> ${newLeads.length} new fire incident leads detected across India.
      </div>

      <h3 style="margin-top: 24px;">📋 Lead Details</h3>
  ...
  try {
    if (process.env.REPORT_EMAIL_FROM && process.env.REPORT_EMAIL_PASSWORD && process.env.AUTHORITY_EMAIL) {
      await transporter.sendMail({
        from: `"FireLeadTracker" <${process.env.REPORT_EMAIL_FROM}>`,
        to: process.env.AUTHORITY_EMAIL,
        subject: `🔥 [FireLeadTracker] ${newLeads.length} New Fire Leads — ${new Date().toLocaleDateString('en-IN')}`,
        html,
      });
    }
